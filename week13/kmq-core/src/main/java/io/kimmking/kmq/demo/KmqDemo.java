...
public class KmqDemo {
    ...
    public static void main(String[] args) {
        ...
        new Thread(() -> {
            while (flag[0]) {
                KmqMessage<Order> message = consumer.pollByUser("userId1", 100);
                if(null != message) {
                    System.out.println("user1 get : " + message.getBody());
                }
            }
        ...
        new Thread(() -> {
            while (flag[0]) {
                KmqMessage<Order> message = consumer.pollByUser("userId2", 100);
                if(null != message) {
                    System.out.println("user2 get : " + message.getBody());
                }
            }
            System.out.println("程序退出。");
        }).start();
